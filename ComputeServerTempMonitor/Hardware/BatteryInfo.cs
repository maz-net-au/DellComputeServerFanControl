...
using static ComputeServerTempMonitor.Hardware.Win32;
...
namespace ComputeServerTempMonitor.Hardware
{
    public class BatteryInformation
    {
        ...
        //BATTERY_STATUS
        public Win32.POWER_STATE PowerState { get; set; }
    ...
    public static class BatteryInfo
    {
        ...
        public static BatteryInformation GetBatteryInformation()
        {
            ...
            try
            {
                ...
                uint reqSize = 0;
                bool retval = Win32.SetupDiGetDeviceInterfaceDetail(deviceHandle, ref deviceInterfaceData, IntPtr.Zero, 0, out reqSize, IntPtr.Zero);

                if (reqSize == 0)
                {
                    throw new Exception("Don't know buffer size for SetupDiGetDeviceInterfaceDetail");
                }
                uint size = reqSize;
                IntPtr deviceDetailData = Marshal.AllocHGlobal((int)reqSize);
                Marshal.WriteInt32(deviceDetailData, (IntPtr.Size == 8) ? 8 : 4 + Marshal.SystemDefaultCharSize);
                retval = Win32.SetupDiGetDeviceInterfaceDetail(deviceHandle, ref deviceInterfaceData, deviceDetailData, size, out reqSize, IntPtr.Zero);
                SP_DEVICE_INTERFACE_DETAIL_DATA deviceObj = Marshal.PtrToStructure<SP_DEVICE_INTERFACE_DETAIL_DATA>(deviceDetailData);

                IntPtr batteryHandle = CreateFile(deviceObj.DevicePath, FileAccess.ReadWrite, FileShare.ReadWrite, FileMode.Open, Win32.FILE_ATTRIBUTES.Normal);
            ...
            IntPtr deviceInfoSet, ref Win32.SP_DEVICE_INTERFACE_DATA deviceInterfaceData, ref Win32.SP_DEVICE_INTERFACE_DETAIL_DATA deviceInterfaceDetailData, int deviceInterfaceDetailSize)
        {
            ...
            {
                int errorCode = Marshal.GetLastWin32Error();
                throw new Exception($"Win32API error: {errorCode}");
                if (errorCode != 0)
                    throw Marshal.GetExceptionForHR(errorCode);
                else
                    throw new Exception(
